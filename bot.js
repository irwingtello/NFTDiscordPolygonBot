    ...
    if (response.data.result?.assets) {
      embedResponses = response.data.result.assets.map((datax) => {
        ...
        const name = collection.collectionName
          ? collection.collectionName.length > 256
            ? collection.collectionName.slice(0, 253)
        ...
        return {
          title: name,
          description: `Address: ${collection.collectionAddress}`,
          image: { url: imageUrl }
        };
      });
    }
...
app.post('/info',express.json(), async (req, res) => {
  ...
  try {
    ...
    if (response.data.result.totalPages === 0) {
      ...
      embedResponses = [
        {
          title:`You currently possess a total of `+ response.data.result.totalItems + ` NFT in your collection.` ,
